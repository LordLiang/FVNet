...
class BramboxDataset(Dataset):
    ...
    def __init__(self, anno_format, anno_filename, input_dimension, class_label_map=None, identify=None, img_transform=None, anno_transform=None, **kwargs):
        ...
        direction_map = ['up', 'left', 'down', 'right']
        ...
        for k, annos in self.annos.items():
            for a in annos:
                ...
                if direction_map is not None:
                    try:
                        a.direction_id = direction_map.index(a.direction)
                    except ValueError as err:
                        raise ValueError(f'{a.direction} is not found in the direction_map') from err
                else:
                    a.direction_id = 0
    ...
    def __getitem__(self, index):
        ...
        # Load
        img = Image.open(self.id(self.keys[index % len(self)]))
        anno = copy.deepcopy(self.annos[self.keys[index % len(self)]])
        random.shuffle(anno)
