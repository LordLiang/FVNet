...
class VOCDataset(data.BramboxDataset):
    def __init__(self, hyper_params):
        ...
        img_tf = data.transform.Compose([rc, rf, it])
...
class VOCTrainingEngine(engine.Engine):
    ...
    def __init__(self, hyper_params):
        ...
        net = models.__dict__[model_name](hyper_params.classes, hyper_params.weights, train_flag=0, clear=hyper_params.clear)
        ...
        self.train_loss = [{'tot': [], 'coord': [], 'conf': [], 'cls': [], 'dst': []} for _ in range(self.nloss)]
    ...
    def process_batch(self, data):
        ...
        for ii in range(self.nloss):
            ...
            self.train_loss[ii]['dst'].append(self.network.loss[ii].loss_dst.item() / self.mini_batch_size)
    ...
    def train_batch(self):
        ...
        for ii in range(self.nloss):
            ...
            dst = mean(self.train_loss[ii]['dst'])
            all_tot += tot
            all_coord += coord
            all_conf += conf
            all_dst += dst
            ...
            if self.classes > 1:
                log.info(f'{self.batch} # {ii}: Loss:{round(tot, 5)} (Coord:{round(coord, 5)} Conf:{round(conf, 5)} Cls:{round(cls, 5)} Dst:{round(dst, 5)})')
            else:
                log.info(f'{self.batch} # {ii}: Loss:{round(tot, 5)} (Coord:{round(coord, 5)} Conf:{round(conf, 5)}) Dst:{round(dst, 5)} )')

        if self.classes > 1:
            log.info(f'{self.batch} # All : Loss:{round(all_tot, 5)} (Coord:{round(all_coord, 5)} Conf:{round(all_conf, 5)} Cls:{round(all_cls, 5)} Dst:{round(all_dst, 5)})')
        else:
            log.info(f'{self.batch} # All : Loss:{round(all_tot, 5)} (Coord:{round(all_coord, 5)} Conf:{round(all_conf, 5)} Dst:{round(all_dst, 5)})')
        self.train_loss = [{'tot': [], 'coord': [], 'conf': [], 'cls': [], 'dst': []} for _ in range(self.nloss)]
