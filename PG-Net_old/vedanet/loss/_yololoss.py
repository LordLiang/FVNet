...
class YoloLoss(nn.modules.loss._Loss):
    ...
    def forward(self, output, target, seen=None):
        ...
        dst = output[:, :, 4:6]
        conf = output[:, :, 6].sigmoid()
        cls = output[:, :, 7:].contiguous().view(nB*nA, nC, nH*nW).transpose(1, 2).contiguous().view(-1, nC)
        ...
        # Get target values
        coord_mask, conf_pos_mask, conf_neg_mask, cls_mask, tcoord, tdst, tconf, tcls = self.build_targets(pred_boxes, target, nH, nW)
        ...
        self.loss_dst = 2.0 * self.distance_scale * (coord_mask*smooth_l1(dst, tdst)).sum()
        ...
        self.info['dst'] = (coord_mask* mse(dst, tdst)).sum().item() / obj_cur
        self.printInfo()

        self.loss_tot = self.loss_coord + self.loss_conf + self.loss_cls + self.loss_dst
    ...
    def __build_targets_brambox(self, pred_boxes, ground_truth, nH, nW):
        ...
        tdst = torch.zeros(nB, nA, 2, nH*nW, requires_grad=False, device=device)
        ...
        for b in range(nB):
            ...
            for i, anno in enumerate(ground_truth[b]):
                ...
                if anno.ignore:
                    ...
                    tdst[b][best_n][0][gj * nW + gi] = float(anno.front_distance / 80.0)
                    tdst[b][best_n][1][gj * nW + gi] = float(anno.back_distance / 80.0)
        ...
        return coord_mask, conf_pos_mask, conf_neg_mask, cls_mask, tcoord, tdst, tconf, tcls

    def printInfo(self):
        info = self.info
        info_str = 'AVG IOU %.4f, Class %.4f, Obj %.4f, No obj %.4f, ' \
                '.5R %.4f, .75R %.4f, Cur obj %3d, All obj %3d, Coord xy %.4f, Coord wh %.4f, Dst %.4f' % \
                (info['avg_iou'], info['class'], info['obj'], info['no_obj'],
                        info['recall50'], info['recall75'], info['obj_cur'], info['obj_all'],
                        info['coord_xy'], info['coord_wh'], info['dst'])
