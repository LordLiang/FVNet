...
class Debugger(object):
  def __init__(self, ipynb=False, theme='black', 
    ...
    elif num_classes == 3 or dataset == 'fv':
      self.names = fv_class_name
    elif num_classes == 2 or dataset == 'fv2':
      self.names = fv2_class_name
  ...
  def add_coco_bbox(self, bbox, cat, conf=1, show_txt=True, img_id='default'): 
    ...
    if bbox.shape[0] == 6:
      txt = '{}{:.1f}[{:.2f},{:.2f}]'.format(self.names[cat], conf, box[4], box[5])
    else:
...
fv_class_name = ['Car', 'Pedestrian', 'Cyclist']

fv2_class_name = ['Car', 'Person']
